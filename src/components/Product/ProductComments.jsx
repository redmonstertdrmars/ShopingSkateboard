...
const ProductComments = () => {
    const { getDetail, productDetails, editCommentProduct } = useContext(productContext)
    ...
    const sendComment = async (e, id, productos) => {
        ...
        if (e.target.parentNode?.firstChild.lastChild.firstChild && e.target.parentNode.firstChild.lastChild.firstChild.value.trim()) {
            ...
            const data = await editCommentProduct(id, productWithComment)
    ...
    const deleteComment = async (index, id, productos) => {
        ...
        const data = await editCommentProduct(id, productWithoutComment)
    ...
    const editComment = async (index, id, productos) => {
        ...
        const data = await editCommentProduct(id, productWithEditedComment)
        ...
        <>
            {productDetails ?
                ...
                <Paper className={classes.paper}>
                    <Paper style={{ color: 'red', maxWidth: "500px" }} spacing={2}>
                        <Grid item xs={12} sm container>
                            <Grid item xs container direction="column" spacing={2}>
                                <Grid item xs>
                                    {product?.comments ? product.comments.map((commentix, index) => (
                                        <div style={marginOfComment(commentix)}>
                                            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start' }}>
                                                <p style={{ color: '#000' }}>{commentix.email}</p>
                                                <p style={{ fontSize: '12px', color: '#000', marginLeft: '10px' }}>{commentix.date}</p>
